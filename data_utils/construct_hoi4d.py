...
def construct_train(h5path:list, dataset_name: str, clip_length: int, n_video: int=-1, n_frame: int=-1, chunk_size: int = 50):
    '''
    ...
    for k, path in enumerate(h5path):
        ...
        for b in tqdm(range(batch+delta)):
            ...
            if b < batch:
                ...
                semantic = np.array(f['semantic'][start_id: start_id+chunk_size, :n_frame])
            else:
            ...
            points_txyz = np.concatenate([np.zeros([video_num, t_video, n_point, 1]), points_xyz], axis=-1)
...
def construct_test(h5path:list, dataset_name: str, clip_length: int, chunk_size: int = 50):
    '''
    Construct dataset and config files.
    Args:
    h5path: list of path to .h5 file of hoi4d dataset.
        ...
        for b in tqdm(range(batch+delta)):
            ...
            for v in range(video_num): 
                ...
                for i in range(N):
                    ...
                    if v % 5 == 0: val_list += f'{dataset_name}/data_{k+1}_{v_}_{i}.npz\n'
                    else: test_list += f'{dataset_name}/data_{k+1}_{v_}_{i}.npz\n'
...
if __name__ == '__main__':
    ...
    clip_length = 8
    train_path = ['/mnt/sdc/wangx/HOI4D/HOI4D_dataset/seg_data_h5/train1_float32.h5',
