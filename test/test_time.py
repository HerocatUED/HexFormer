...
def timing(pcd, depth=10, n_frame=16, need_neigh=False):
    ...
    end_time = time.time()
    print(
        f"Depth: {depth}, \tframe: {n_frame}, \thextree building time: {(mid_time - start_time)}"
        + (
            f", \tneighbour constructing time: {(end_time - mid_time)}"
            if need_neigh
            else ""
        )
    )
...
if __name__ == "__main__":
    test_timing(
        data_path="../data/points.npy", label_path="../data/semantic.npy", video_id=0
    )
