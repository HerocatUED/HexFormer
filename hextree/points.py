...
# Hextree version modified by Ruihuan Wang
# --------------------------------------------------------
...
class Points:
    ...
    Args:
        points (torch.Tensor): The txyz coordinates of the points with a shape of 
    ...
    def scale_xyz(self, factor: torch.Tensor):
        r''' Rescale the pointcloud on xyz.
    ...
    def scale_txyz(self, factor: torch.Tensor):
        r''' Rescale the pointcloud on txyz.

        Args:
            factor (torch.Tensor): The scale factor with shape :obj:`(4,)`.
        ''' 
        ...
        if self.normals is not None and non_uniform:
            ifactor = 1.0 / factor
            self.normals = self.normals * ifactor[1:]
            norm2 = torch.sqrt(torch.sum(self.normals ** 2, dim=1, keepdim=True))
            self.normals = self.normals / torch.clamp(norm2, min=1.0e-12)
    ...
    def translate_xyz(self, dis: torch.Tensor):
        r''' Translates the pointcloud on xyz.
    ...
    def translate_txyz(self, dis: torch.Tensor):
        r''' Translates the pointcloud on txyz.

        Args:
            dis (torch.Tensor): The displacement with shape :obj:`(4, )`.
        ''' 
    ...
    def flip(self, axis: str = 'x'):
        r''' Flips the pointcloud along the given :attr:`axis`.
    ...
    def clip_xyz(self, min: float = -1.0, max: float = 1.0, eps: float = 0.01):
        ...
        Args:
            ...
            eps (float): The margin.
        '''
        mask = self.inbox_mask(min + eps, max - eps)
    ...
    def bbox_xyz(self):
        r''' Returns the xyz bounding box.
        '''
    ...
    def normalize_xyz(self, bbmin: torch.Tensor, bbmax:torch.Tensor,
                      scale: float = 1.0):
        r''' Normalizes the pointcloud to :obj:`[-scale, scale]`.
        ...
        self.points[:, 1:] = (self.points[:, 1:] - center) * (2.0 * scale / box_size)
