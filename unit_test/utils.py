...
def points_3Dto4D(points_xyz: np.ndarray):
    ...
    points_txyz = np.concatenate(
        [np.zeros([n_video, t_video, n_point, 1]), points_xyz], axis=-1
    )
    points_txyz[:, :, :, 0] += (np.arange(t_video) + 1)[None, :, None]

    points_txyz = torch.Tensor(
        points_txyz.reshape([n_video, -1, 4])
    )  # (n_video, t_video * n_point, 4)
    return points_txyz


def read_points(data_path: str, label_path: Optional[str] = None):
    ...
    pcds = [
        Points(
            points=points_txyz[i], labels=semantic[i] if semantic is not None else None
        )
