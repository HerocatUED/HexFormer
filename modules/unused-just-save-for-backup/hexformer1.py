...
class PatchEmbed(torch.nn.Module):
    ...
    def __init__(self, in_dim: int, dim: int, num_stages: int, init_depth: int, **kwargs):
        super().__init__()
...
class HexFormer(torch.nn.Module):
    ...
    def __init__(self, in_channels: int,
                 ...
                 stem_down: int = 2, init_depth:int = 10, **kwargs):
        super().__init__()
        self.patch_size = patch_size
        self.dilation = dilation
        self.nempty = nempty
        ...
        # Embdedding
        self.patch_embed = PatchEmbed(in_channels, channels[0], stem_down, init_depth)
        ...
        self.feature_down = torch.nn.ModuleList([MLP(channels[i], int(
            (channels[i] + channels[i-1]) / 2), channels[i-1]) for i in range(self.num_stages-1, 0, -1)])
        self.upsample = HextreeAvgUnpoolXYZ()
        self.decoder_norms = torch.nn.ModuleList([
            torch.nn.LayerNorm(channels[i]) for i in range(self.num_stages-2, -1, -1)])
        # unPE
        head_channels = [int(channels[0] * 2**i) for i in range(-self.stem_down, 1)]
        self.head_up = torch.nn.ModuleList([MLP(head_channels[i-1], int((head_channels[i-1] + channels[0]) / 2), 
            channels[0]) for i in range(self.stem_down, 0, -1)])
        self.head_norm = torch.nn.LayerNorm(channels[0])
    ...
    def forward(self, data: torch.Tensor, hextree: Hextree, depth: int):
        ...
        for i in range(self.num_stages - 1):
            depth_i = depth + i + 1
            data = self.feature_down[i](data)
            data = self.upsample(data, hextree, depth_i-1, depth_i)
            data = self.decoders[i](data, hextree, depth_i)
            data = features[depth_i] + data
            data = self.act(self.decoder_norms[i](data))
        # PE upsample
        for i in range(self.stem_down):
            depth_i = target_depth - self.stem_down + i
            data = self.upsample(data, hextree, depth_i, depth_i + 1) + self.head_up[i](features[depth_i + 1])
            data = self.act(self.head_norm(data))
        return data
