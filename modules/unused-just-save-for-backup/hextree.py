...
class Hextree:
    ...
    def search_txyzb(self, query: torch.Tensor, depth: int, nempty: bool = False):
        ...
        key = txyz2key(query[:, 0], query[:, 1], query[:, 2],
                       query[:, 3], query[:, 4], depth)
    ...
    def reset(self):
        ...
        self.batch_nnum_nempty = torch.zeros(
            num, batch_size, dtype=torch.int64)
    ...
    def build_hextree(self, point_cloud: Points):
        ...
        points[points == 2 * scale] = 2 * \
            scale - 1  # 2 ^ depth -> 2 ^ depth - 1
        ...
        for d in range(self.depth, self.full_depth, -1):
            ...
            pkey = node_key >> 4
            pkey, pidx = torch.unique_consecutive(
                pkey, return_inverse=True, dim=0)

            # augmented key
            key = ((pkey.unsqueeze(-1) << 4) +
                   torch.arange(16, device=self.device)).view(-1)
            self.keys[d] = key
    ...
    def hextree_grow_full(self, depth: int, update_neigh: bool = False):
        ...
        bs = torch.arange(self.batch_size, dtype=torch.long,
                          device=self.device)
    ...
    def construct_neigh(self, depth: int):
        ...
        if depth <= self.full_depth:
            ...
            neigh = txyz2key(txyz[:, 0], txyz[:, 1], txyz[:, 2],
                             txyz[:, 3], depth=depth)   # (N * 81)

            bs = torch.arange(
                self.batch_size, dtype=torch.int64, device=self.device)
            # (1, N * 81) + (B, 1) -> (B, N * 81)
            neigh = neigh.unsqueeze(0) + bs.unsqueeze(1) * nnum

            bound = 1 << depth
            invalid = torch.logical_or(
                (txyz < 0).any(1), (txyz >= bound).any(1))
            neigh[:, invalid] = -1
...
def merge_hextrees(hextrees: List['Hextree']):
    ...
    for d in range(hextree.depth + 1):
        ...
        running_length = torch.tensor(
            0, dtype=torch.long, device=hextree.device)
        for i in range(hextree.batch_size):
